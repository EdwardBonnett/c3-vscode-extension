...
export function parseStruct(content: string, modules: {[path: string]: Module}, mpth: string): Module {
    ...
    tsm.Matching.visit(mod, x => {
        if (x.kind === ts.SyntaxKind.VariableDeclaration) {
            x.forEachChild(c => {
                if (c.kind === ts.SyntaxKind.FunctionExpression) {
                    ...
                    let isAsync = !!(c.modifiers || [] as any).find((m: any) => m.kind === ts.SyntaxKind.AsyncKeyword);
                    const name = (x as ts.FunctionDeclaration).name?.escapedText as string;
                    (c as any).parameters.forEach((param: any) => {
                        params.push({
        ...
        if ( x.kind === ts.SyntaxKind.ImportDeclaration ) {
            ...
            tsm.Matching.visit(localMod, y => {
                ...
                if (y.kind === ts.SyntaxKind.StringLiteral) {
                    ...
                    localImport.absPathNode = localAbsPath as never[];
        ...
        if (c) {
            ...
            c.members.forEach(x => {
                if (x.kind === ts.SyntaxKind.MethodDeclaration || x.kind === ts.SyntaxKind.MethodSignature) {
            ...
            if (c.typeParameters) {
                c.typeParameters.forEach(x => {
                    ...
                    if (!x.constraint) {
                        ...
                        clazz.typeParameterConstraint.push((x.constraint as unknown as { typeName: any })["typeName"] ? (x.constraint as unknown as { typeName: any })["typeName"]["text"] : null);
                    }
                });
            }
...
function buildField(f: ts.PropertyDeclaration, path: string): FieldModel {
    return {
        name: (f.name as {text: string}).text || (f.name as any).expression.text,
        type: buildType(f.type!, path)!,
        annotations: ((f.name as {text: string}).text || (f.name as any).expression.text).charAt(0) === "$" ? buildInitializer(f.initializer!) : [],
        valueConstraint: ((f.name as {text: string}).text || (f.name as any).expression.text).charAt(0) !== "$" ? buildConstraint(f.initializer!) : null,
...
function buildParameter(f: ts.ParameterDeclaration, content: any, path: string): ParameterModel {
    ...
    return {
        name: (f.name as {text: string}).text,
        start: f.pos,
        end: f.end,
        text: text,
...
function buildConstraint(e: ts.Expression): Constraint | null{
    if (!e) {
        return null;
    }
...
function buildAnnotation(e: ts.Expression): Annotation {
    if (e.kind === ts.SyntaxKind.CallExpression) {
        ...
        call.arguments.forEach(x => {
            a.arguments.push(parseArg(x) as never);
        });
        return a;
    } else {
...
export function parseArg(n: ts.Expression): any {
    ...
    if (n.kind === ts.SyntaxKind.PropertyAccessExpression) {
        ...
        return parseArg(pa.expression) + "." + parseArg(pa.name as ts.Expression);
    }
...
function parseName(n: ts.Expression): string {
    ...
    if (n.kind === ts.SyntaxKind.PropertyAccessExpression) {
        ...
        return parseName(m.expression) + "." + parseName(m.name as ts.Expression);
    }
...
export function buildType(t: ts.TypeNode, path: string): TypeModel | null {
    if (!t) {
        return null;
    }
    ...
    if (t.kind === ts.SyntaxKind.StringLiteral || t.kind === ts.SyntaxKind.LiteralType) {
        switch ((t as any).literal.kind) {
            case ts.SyntaxKind.FalseKeyword:
                return basicType('false', null);
            case ts.SyntaxKind.TrueKeyword:
                return basicType('true', null);
            case ts.SyntaxKind.PrefixUnaryExpression:
                    return basicType((t as any).literal.operand.text, null);
            default:
                return basicType((t as any).literal.text || 'null', null);
        }
    }
    else {
