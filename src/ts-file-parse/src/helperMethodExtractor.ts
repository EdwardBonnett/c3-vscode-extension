...
export function getHelperMethods(srcPath: string): HelperMethod[] {
    ...
    tsm.Matching.visit(mod, x => {
        ...
        if (node.kind === ts.SyntaxKind.FunctionDeclaration) {
            ...
            var args = node.parameters ? node.parameters.map((a: any) => readArg(a, srcPath)) : [];
...
var refineComment = function (comment: any) {
    return comment.replace(/^\s*\/\*+/g, "").replace(/\*+\/\s*$/g, "").split("\n")
        .map((x: any) => x.replace(/^\s*\/\//g, "").replace(/^\s*\* {0,1}/g, "")).join("\n").trim();
};
...
function getMeta(node: any, content: string): Meta | null {
    ...
    var comment = cRange.map((x: any) => content.substring(x.pos, x.end)).join("\n");
